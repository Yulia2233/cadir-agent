...
Run `pnpm test:integration` for the acceptance-catalog and deployment contract suites. The catalog check imports all 457 authoritative IDs from `../to_test.md`; it does not mark a case passed until an implementation-specific test or recorded manual execution supplies evidence. After building the Runner as `cadir-runner:dev`, run `pnpm test:runner-container` to verify the non-root identity, pinned SDK and Skill, read-only root filesystem, and network-disabled execution mode against a real Linux container. Run `pnpm test:runner-cad` to import the CAD kernel, create a GraphSession model, strictly replay canonical JSON, and export nonempty STEP/STL files inside that same image.
